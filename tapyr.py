...
class Tapir:
    ...
    def schedule(self, plugin, arguments, relaunch=False):
        ...
        if r.ok:
            ...
            # this block until all task are finished, this block the entire server so must be used for debug purpose
            # only
    ...
    def timeline(self, after, before, name=False, path=False, attributes=False, children=False):
        ...
        r = self.requests.post(url, json={'after': after, 'before': before,
                                          'option': {"path": path, "name": name, "attributes": attributes,
                                                     "children": children}})
        if r.ok:
            return json.loads(r.content.decode('UTF-8'))
        else:
